...
namespace Shin_UnityLibrary
{
    public static class EnumBuilder
    {
        ...
        public static Enum BuildEnumFromClasses(List<string> classNames, string enumName)
        {
            Type underlyingType = typeof(int); // 列挙型の基底型を指定します。ここではint型を使用しています。

            TypeBuilder typeBuilder = CreateTypeBuilder(enumName, underlyingType);
            ...
            return (Enum)Enum.Parse(enumType, classNames[0]); // 初期値として最初のクラス名を使用します。
        }

        private static TypeBuilder CreateTypeBuilder(string enumName, Type underlyingType)
        {
        ...
        private static FieldBuilder[] CreateEnumFields(List<string> classNames, TypeBuilder typeBuilder, Type underlyingType)
        {
