...
namespace Shin_UnityLibrary
{
    public class FoldOutAttribute : PropertyAttribute
    {
        ...
        public FoldOutAttribute(string name, bool init = false, int space = 0)
        {
            this.init = init;
            this.name = name;
            this.space = space;
    ...
    public class FoldOutDrawer : PropertyDrawer
    {
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (FoldOutAttribute.init)
            {
                if (foldDict[FoldOutAttribute.name]) return Height(property, label) - 16f + FoldOutAttribute.space;
            }
            if (foldDict[FoldOutAttribute.name]) return Height(property, label) - 20f + FoldOutAttribute.space;
            return 0;
